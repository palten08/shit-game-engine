...
void rendering_pipeline_worker(void *arg) {
    ...
    for (int i = job->entity_start; i < job->entity_end; i++) {
        ...
        Material *material = &job->scene->asset_library.materials[mesh_data->material_id];
        ...
        for (int t = 0; t < mesh_data->triangle_count; t++) {
            ...
            switch (material->shading_model) {
                case SHADING_FLAT:
                    // Flat shading only uses face normals
                    // Order of operations: model transform -> calculate face normal in world space -> calculate lighting -> apply lighting to material color
                    // Transform the face normal to world space
                    Vector3f face_normal = mesh_data->face_normals[t];
                    Vector3f world_space_normal = {
                        transform->model_matrix.m[0][0] * face_normal.x + transform->model_matrix.m[0][1] * face_normal.y + transform->model_matrix.m[0][2] * face_normal.z,
                        transform->model_matrix.m[1][0] * face_normal.x + transform->model_matrix.m[1][1] * face_normal.y + transform->model_matrix.m[1][2] * face_normal.z,
                        transform->model_matrix.m[2][0] * face_normal.x + transform->model_matrix.m[2][1] * face_normal.y + transform->model_matrix.m[2][2] * face_normal.z
                    };
                    ...
                    // Calculate diffuse lighting using the Lambertian reflectance model
                    ...
                    // Apply the calculated brightness to the material's diffuse color to get the final vertex color for flat shading
                    RGBVector3f flat_shading_color = (RGBVector3f){
                        material->diffuse_color.r * brightness * job->scene->directional_light.color.r,
                        material->diffuse_color.g * brightness * job->scene->directional_light.color.g,
                        material->diffuse_color.b * brightness * job->scene->directional_light.color.b
                    };

                    // Apply the same color to all vertices of the triangle for flat shading
                    // There will be no per-vertex interpolation
                    vertex_colors[0] = flat_shading_color;
                    vertex_colors[1] = flat_shading_color;
                    vertex_colors[2] = flat_shading_color;
                    break;
                case SHADING_PHONG:
                    // Phong shading uses vertex normals for per-vertex lighting calculations, which are then interpolated across the triangle during rasterization
                    // Order of operations: model transform -> transform vertex normals to world space -> calculate lighting at each vertex -> output vertex color
                    for (int v = 0; v < 3; v++) {
                        // Get a vertex normal from the mesh data and transform it to world space
                        ...
                        Vector3f world_space_vertex_normal = {
                            transform->model_matrix.m[0][0] * vertex_normal.x + transform->model_matrix.m[0][1] * vertex_normal.y + transform->model_matrix.m[0][2] * vertex_normal.z,
                            transform->model_matrix.m[1][0] * vertex_normal.x + transform->model_matrix.m[1][1] * vertex_normal.y + transform->model_matrix.m[1][2] * vertex_normal.z,
                            transform->model_matrix.m[2][0] * vertex_normal.x + transform->model_matrix.m[2][1] * vertex_normal.y + transform->model_matrix.m[2][2] * vertex_normal.z
                        };
                        world_space_vertex_normals[v] = world_space_vertex_normal;

                        // Apply the computed color to this vertex
                        // This will be interpolated across the triangle during rasterization for smooth shading
                        vertex_colors[v] = material->diffuse_color; // Start with the material's diffuse color, and then the rasterizer will apply lighting calculations per pixel for Phong shading
                    }
                    break;
                case SHADING_PBR:
                    // We'll do this later
            ...
            for (int c = 0; c < clipping_result.vertex_count - 2; c++) {
                ...
                render_triangle.vertex_colors[0] = clipping_result.vertex_colors[0];
                render_triangle.vertex_colors[1] = clipping_result.vertex_colors[c + 1];
                render_triangle.vertex_colors[2] = clipping_result.vertex_colors[c + 2];
                render_triangle.world_space_vertex_positions[0] = clipping_result.world_space_vertex_positions[0];
                render_triangle.world_space_vertex_positions[1] = clipping_result.world_space_vertex_positions[c + 1];
                render_triangle.world_space_vertex_positions[2] = clipping_result.world_space_vertex_positions[c + 2];
                render_triangle.world_space_vertex_normals[0] = clipping_result.vertex_normals[0];
                render_triangle.world_space_vertex_normals[1] = clipping_result.vertex_normals[c + 1];
                render_triangle.world_space_vertex_normals[2] = clipping_result.vertex_normals[c + 2];
                render_triangle.flat_shading_color = vertex_colors[0]; // For flat shading, all vertices have the same color, so we can just use the first one
                render_triangle.material = material;
    ...
    for (int i = 0; i < actual_job_count; i++) {
        if (jobs[i]->render_triangles && !allocation_failed && jobs[i]->triangle_count > 0) {
            ...
            if (!combined_triangles) {
                ...
                render_list.triangles = combined_triangles;
